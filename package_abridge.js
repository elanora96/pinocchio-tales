...
const bpath = fs.existsSync("./themes") ? "themes/abridge/" : "";
...
async function sync() {
  ...
  let checkPackageVersion = function (content) {
    ...
    return [...matches.matchAll(/"(\w+-\w+|\w+)": "[^0-9]*([0-9])/g)]
      .map((match) => ({
        // Extract all packages and their major version number (aka for breaking changes which need an update)
        name: match[1],
  ...
  const packageVersionSubmodule = checkPackageVersion(
    submodulePackageJsonContent,
  );
  if (
    JSON.stringify(packageVersionLocal) !==
    JSON.stringify(packageVersionSubmodule)
  ) {
    console.log(
      "\x1b[31m%s\x1b[0m",
      "warning:",
